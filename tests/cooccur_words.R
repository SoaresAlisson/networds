...
test_that("cooccur function works correctly", {
  ...
  test_that("invalid token_by throws error", {
    expect_error(
      cooccur_words(sample_text, token_by = "word", sw = ""),
...
test_that("cooccur function with lower parameter works correctly", {
  ...
  test_that("lower defaults to TRUE", {
    result_default <- cooccur_words(
      sample_text,
      token_by = "sentence",
      sw = "",
      count = TRUE
    )
    result_explicit <- cooccur_words(
      sample_text,
      token_by = "sentence",
      sw = "",
      count = TRUE,
...
test_that("cooccur edge cases", {
  ...
  test_that("long text processes correctly", {
    long_text <- paste(
      rep("This is a test sentence with multiple words for testing.", 10),
      collapse = " "
    )
    result <- cooccur_words(
      long_text,
...
test_that("pair order is consistent", {
  text <- "Word1 word2 word3."
  text2 <- "Word2 word1 word3."
  result <- cooccur_words(text, token_by = "sentence", sw = "", count = TRUE) |>
    dplyr::arrange(n1, n2)
  result2 <- cooccur_words(text2, token_by = "sentence", sw = "", count = TRUE)
  # put the node order alphabetically
  result2 <- result2 |>
    dplyr::mutate(test = n1 > n2) |>
    dplyr::mutate(n1t = ifelse(test, n2, n1)) |>
...
test_that("Sentence with only two words works as expected", {
  text <- "Word1 word2"
  expected <- tibble::tibble(n1 = "word1", n2 = "word2", n = 1L)
  result <- cooccur_words(text, token_by = "sentence", sw = "")
...
testthat::test_that("list output", {
  txt <- c("Bla ble bli. Ble bli blo", "Lorem ipsum dolor sit amet.")
  # text <- txt
  result <- cooccur_words(txt, output = "lst")
  expect_type(result, "list")
  expect_length(result, 2)
  expect_equal(ncol(result[[1]]), 3)
  # all_col_length
  expect_true(all(unlist(lapply(result, ncol)) == 3))
  expect_equal(length(txt), length(result))

  # erase element with one element
  txt <- c("Bla ble bli. Ble bli blo", "Lorem ipsum dolor sit amet.", "Nullam")
  # text <- txt
  result <- cooccur_words(txt, output = "lst")
  expect_type(result, "list")
  expect_equal(ncol(result[[1]]), 3)
  ...
  result <- cooccur_words(txt, output = "lst", count = FALSE)
  expect_type(result, "list")
  expect_equal(ncol(result[[1]]), 2)
  expect_true(all(unlist(lapply(result, ncol)) == 2))
})


# ....... Test output df2: df pair frequency per document ......

testthat::test_that("df output", {
  text <- c(
    "The first document. Lorem ipsum",
    "The second document foo bar. Lorem bar foo."
  )
  DFresult <- text |> cooccur_words(output = "df2")
  # n of docs
  ...
  # stopwods works
  my_sw <- c("the")
  DF <- text |> cooccur_words(my_sw, output = "df2")
  expect_false(any("the" == c(DF$n1, DF$n2)))
})
