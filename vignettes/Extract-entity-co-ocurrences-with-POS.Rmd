---
title: "03 Extract entity co-ocurrences with Part-of-speech Tagging"
check_title: FALSE
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{03 Extract entity co-ocurrences with POS}
...
If you are running the script directly in the command line, outside an IDE like RStudio, you must specify the global environment variable of yout browser. In my case, I'm using Firefox.
```{r browser, echo=TRUE}
...
graph_wiki$graphs |> plot_graph_i()
```
...
graph$graphs |> plot_graph_i()
```

To get the graph of entities and nouns:

```{r police -cardinal + viz_graph, eval=FALSE}
...
```{r police -cardinal + viz_graph 2, eval=FALSE}
plot_graph_i(graph_ppn$graphs)
```



```{r police -cardinal + plot_pos_graph, eval=FALSE}
...
How {networds} uses Spacy package to tag the words, the user must initialize the model for the language.
...
To load the Portuguese model, use with `spacyr::spacy_initialize(model = "pt_core_news_lg")`.
