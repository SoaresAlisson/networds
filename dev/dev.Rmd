...
```{r ll}
library(devtools)
sto::ll("dplyr spacyr udpipe sto rsyntax tibble tidygraph ggraph networds")
options(browser = "firefox")
# detach("package:txtnet", unload = T)
detach("package:networds", unload = T)
unloadNamespace("networds")
...
devtools::build_vignettes("vignettes/02-Extract-entity-co-ocurrences-with-POS.Rmd")
...
devtools::clean_vignettes()
devtools::build(vignette ss = FALSE)
...
pkgdown::build_reference() # build references
pkgdown::build_articles() # build 
...
# @param DT if TRUE, use data.table instead of dplyr
#'
#' @export
#'
#' @examples
...
count_graph <- function(graph, loop = FALSE) {
  ...
  # if(!DT){
    gr <- graph |>
      ...
      # label = n2,
  # } else {
  #     # Converts the data frame DT into a data.table for efficient operations.
  #     data.table::setDT(graph)
  #       # Count occurrences of 'n1' and 'n2' and sort by count in descending order
  #     DT[, .(n = .N), by = .(n1, n2)][order(-n)]
  #   }
...
arq <- "~/Documentos/Programação/R/cebrap/analise_tribunais/data/uber-agregado-acordaos.Rds"
file.exists(arq)
# file.exists("arq")
...
get_graph_from_txt
extract_relation
graph_subs - make accept named vector

delete?
split_graph, or transform it into export to gephi
