...
get_pairs <- function(pos, loop=FALSE) {
  entities_by_txt <-
    if (loop){
    pos |> 
        dplyr::group_by(doc_id) |> dplyr::summarise(entities = list(entity)) 
    } else {
    pos |>
      dplyr::group_by(doc_id) |> dplyr::summarise(entities = list(unique(entity))) 
    }

  comb_list <- entities_by_txt$entities |> 
    as.list() |>
    lapply(combn, 2, simplify=FALSE) # |> unlist()
...
filter_by_query(txt_wiki[1:3], "kill") |> parsePOS()  |> get_pairs(loop=TRUE)
filter_by_query(txt_wiki[1:3], "kill") |> parsePOS()  |> get_cooc_entities(loop=TRUE)
