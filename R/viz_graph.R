...
viz_triplets_i <- function(
  ...
  graph_df <- to_head_or_not_not_head(graph_df, head_n)
  ...
  graph_data <- graph_df |> # Replace node names with indices in the graph data
    dplyr::mutate(
      from = purrr::map_chr(
        from,
        ~ node_mapping$id[node_mapping$label == .x] |> as.character()
      ),
      to = purrr::map_chr(
        to,
        ~ node_mapping$id[node_mapping$label == .x] |> as.character()
  ...
  visNetwork::visNetwork(
    ...
    visNetwork::visNodes(size = node_size) |>
    visNetwork::visEdges(
      color = list(color = "rgba(0,0,255,0.15)", highlight = edge_color)
    )
}

# TODO - viz graph static
# triplet_viz
