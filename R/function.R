...
sort_columns_alphabetically <- function(DF) {
  DF$temp1 <- ifelse(DF$n1 < DF$n2, DF$n1, DF$n2)
  DF$temp2 <- ifelse(DF$n1 < DF$n2, DF$n2, DF$n1)
...
group_entities <- function(DF) {
  ...
  dplyr::group_by(
    name = cumsum(0 + !(dplyr::lag(name, default = TRUE) & name)),
  ) |>
    dplyr::mutate(
      name = token |>
        # unique() |>
...
extract_entities_v <- function(DF, rgx_entities = "^(PER(SON)?|ORG|GPE)_") {
  ...
  DF |>
    ...
    cbind(L = DF$name) |>
    dplyr::mutate(L2 = L == T & dplyr::lag(L) == T, L3 = is.na(L2)) |>
    dplyr::filter(L3) |>
    # select(-L2, -L3, -L) |>
    dplyr::pull(name)
}
...
#' Collapse ADP
#'
#' In POS, collapse upos ADP, if the previous and the next pos are NOUN or PROPN.
#' It is used in the pipeline to extract proper names like "United States of America"
#'
...
# DELETE fun rename_cols?
#' rename cols from dplyr::count
#'
#' @description
#' rename cols froum dplyr::count to use with other functions, renaming to
#' "from", "to" and "value" for n (frequency.)'
#'
#' @export
#'
#' @examples
...
rename_cols <- function(df) {
  df |>
    dplyr::rename(from = n1, to = n2, value = n)
}
...
#'
#' @examples
#' DF <- data.frame(from = c("Amanda", "Bruno", "Carlos", "Daniel"),
...
#' function to delete from list all list elements with less than 2 elements
#'
#' @description
#' used inside cooccur_words(), to erase empty elements that do not form a pair.
#'
#' @noRd
filter_nested_list <- function(x, min_length = 2) {
  if (is.list(x)) {
    # Recursively process each element
    x <- lapply(x, filter_nested_list, min_length)
    # Remove NULL elements and empty lists
...
#'
#' @description
#' show the displays available to be used in plot functions.
#' dh: Davidson and Harels algorithm,
#' mds:multidimensional scaling of nodes
#' fr: algorithm of Fruchterman and Reingold. igraph::with_fr()
#' kk: Kamada and Kawai igraph::with_kk(). Kamada, T. and Kawai, S.: An Algorithm for Drawing General Undirected Graphs. _Information Processing Letters_, 31/1, 7-15, 1989.
#' drl: force directed algorithm from the DrL toolbox igraph::with_drl()
...
#' Names of dataframes, to be used inside functions in arg %in% df_names
#' @noRd
df_names <- c("df", "dataframe", "tbl", "tibble")
lst_names <- c("list", "lst")
