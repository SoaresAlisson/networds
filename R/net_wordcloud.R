...
#' @param text_contour_color if empty (default) no contour is used. If the color
#' is specified, so the contour is used.
...
net_wordcloud <- function(
  ...
  #     " nodes not found in the text provided. Make sure that text parameter is the same used for graph extraction. Nodes not found: \n", paste(vertices_not_in_freq, collapse = ", ") ))}
