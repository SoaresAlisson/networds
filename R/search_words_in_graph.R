...
#' It is useful to shrink the amount of text to be processed, for example, when 
#' expanding the graph from their neighbours.
#'
...
filter_by_query <- function(txt, query, i_c = TRUE, by_sentence = TRUE, unlist = FALSE, msg = TRUE) {
...
#'
#' @description
#' given a dataframe with a column (list) with POS, returns a list with three
#' elements: 1) a tibble with the frequency of graphs; 2) a tibble with the 
#' isolated nodes, i.e. nodes without any connection. 3) a tibble with the 
#' individual frequency of each node.
#' If list element has only one element, it is removed.
#'
#' @param co-occurrence_list a dataframe generated by get_pairs()
...
get_cooc_entities <- function(pos, loop = FALSE, freq = TRUE, lower_case = FALSE) {
...
#' get graph (co-occurrence of entities and Nouns)
#'
#' @description
#' from a POS dataframe (using `filter_by_query() |> parsePOS()` ) get the pairs of co-occurrences.
#'
