...
#'
#' @description
#' given a data frame with a column (list type) with POS, returns a list with three
...
graph_from_cooccurrence <- function(
  ...
  if (not_adequated_colnames) {
    if (any(column_names %in% "entities")) {
      ...
      stop(
        "Column 'terms' or 'entities' not found. Did it as generated with get_pairs() function?"
      )
    }
  }
  ...
  if (all_nodes_are_lonely) {
    stop("All nodes are solitary. No graph pairs detected.")
  }
  ...
  if (strip_rgx != "") {
    comb_list <- gsub(x = comb_list, strip_rgx, "")
  }
  ...
  # print result
  list(
    graphs = tibble::as_tibble(edgesDF),
    isolated_nodes = tibble::as_tibble(isolated_nodes),
    nodes = all_nodes
  )
}
...
#'
#' @description
#' from a POS dataframe (using filter_by_query() |> parsePOS() or
#' by spacyr::spacy_parse) get the pairs of entities
#'
#' @param pos a dataframe generated by `filter_by_query() |> parsePOS()` or by `spacyr::spacy_parse()`
